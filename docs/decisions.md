...
| 30 | **A player with nothing on record gets nulls, not zeros, and a `has_*_record` flag says which** | 2026-09-21 | Measured: 61.2% of panel rows have an injury on record by their anchor and 93.1% a transfer. A zero in `days_injured_12m` would tell the model "never hurt" about the 27,242 rows (38.8%) with nothing recorded. A player who does have a history and a quiet year still scores a real 0, so flag and value carry different information: flag true with 0 days means measured and fine, flag false means unknown. Same shape as trap 4 and trap 5. The flag conflates "not tracked" with "not yet hurt" and no version of it can separate the two — see #34. |
| 31 | **An injury spell is clipped to the 12-month window at both ends, and overlapping spells are unioned** | 2026-09-21 | `days_missed` covers the whole spell, so a player who goes down a fortnight before the anchor and is out for a year would carry that year into a feature dated 1 July — the future, straight into a training column. Clipping at the anchor fixes it. Summing the clipped spells then broke a different way: Transfermarkt logs the knock and the operation that follows as separate overlapping rows, which double-counted and produced **609 days inside a 365-day window** on the real panel. Taking the union of intervals brings the maximum to 366 (a leap-year window). Caught by validation, not by the first test pass. |
| 32 | **`transfer_fee` of 0 is read as unrecorded, not as a free transfer** | 2026-09-21 | Measured: 1,061,443 of 1,101,440 rows (96.4%) carry fee 0, every loan and every return from loan among them, and only 39,997 rows carry a positive fee. Averaging those zeros in would price the median move at nothing. `transfer_fee_12m` is therefore null unless a positive fee exists, which leaves it 12.8% filled. Sanity-checked: the largest values in the panel are Neymar €222M (2018), Mbappé €180M (2018), Bellingham €127M (2023) and Wirtz €125M (2025). |
| 33 | **The injury group goes into the task 15 ablation as one block, and its drift is declared up front** | 2026-09-21 | Measured per anchor: the share of rows with an injury in the prior year runs 0.253 (2013) to 0.514 (2021), and the share with anything on record 0.410 to 0.752. Training stops at 2020 and testing starts at 2022, so the drift crosses the split. The features stay — they correlate with `y_h1` about as strongly as anything in slot 3 (`injury_spells_12m` −0.170, `days_injured_12m` −0.120) — but any gain they show must be read against the coverage change, not reported as football. Sixth trap in `docs/data_coverage.md`. |
| 34 | **`has_injury_record` and `has_transfer_record` are as-of the anchor, not as-of the scrape** | 2026-09-21 | Caught by the task-12 behavioural test, which rebuilds the panel with post-anchor events added and asserts no feature column moves. The first version asked whether the player appears anywhere in the file, so a player first injured in August arrived at a 1 July anchor already flagged as having an injury record — the future wearing a coverage flag, and it dragged the whole null-versus-zero encoding with it. Now only rows dated on or before the anchor count. The cost is that the flag no longer means "tracked"; it means "has a record by now", which rises with scrape coverage and with career length together. No leak-free version can separate those, so the ambiguity is documented rather than hidden. |
| 35 | **Every feature module declares its source columns in `leakage.FEATURE_SOURCES`, and the test suite fails if one does not** | 2026-09-21 | Task 12. The name check catches a snapshot column; it cannot catch reading a real column at the wrong date, which is how both leaks in this slot actually happened (#31, #34). So the declaration is paired with a behavioural test that builds the panel twice — once clean, once with a revaluation, a record transfer, a long injury and another season of football all dated after the anchor — and asserts every non-target column is identical. A guard test also asserts the pollution changes `y_h1`, so the leakage test cannot pass by doing nothing. |
