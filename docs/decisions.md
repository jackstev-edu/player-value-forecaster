...
| 26 | **`years_of_history` is suspected of being close to a second age column** | 2026-09-21 | First signal, to be settled in the task 15 ablation rather than now: against `y_h1`, `age` correlates −0.394 and `years_of_history` −0.374 — close enough to be worth checking whether the two are collinear. They are not identical (a late-scouted 25-year-old has little history), so the column stays until the ablation says otherwise. For reference the two history features that look genuinely additive are `value_vs_peak` (+0.292) and `value_change_12m` (+0.247). |
