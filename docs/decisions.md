...
| 30 | **An untracked player gets nulls, not zeros, and a `has_*_record` flag says which** | 2026-09-21 | `player_injuries` reaches 79.1% of panel rows and `transfer_history` 93.1%. A zero in `days_injured_12m` would tell the model "never hurt" about 14,647 rows (20.9%) that are simply absent from the file. Tracked players with a quiet year still score a real 0, so the flag and the value carry different information: flag true with 0 days means measured and fine, flag false means unknown. Same shape as trap 4 and trap 5. |
